...
"""
    ...
    for i, t in enumerate(raw_tasks):
        user_prompt += f"Task #{i+1} (Dự án: {t.get('project_code', 'Chưa rõ')}):\n"
        user_prompt += f"- Room: {t.get('room_name', '')}\n"
...
def edit_task_with_ai(current_content, user_request, provider, api_key):
    ...
    and returns the newly edited content.
    """
...
- Dưới mỗi task sẽ có các trường như '- **Project**:' và '- **Title**:'.
...
Hãy thay đổi chính xác nội dung/trường được yêu cầu (ví dụ: sửa trường Project hoặc sửa trường Title) của đúng task đó.

Yêu cầu cực kỳ quan trọng:
1. Giữ nguyên cấu trúc Markdown tổng thể của toàn bộ tài liệu ban đầu.
2. TUYỆT ĐỐI không thay đổi, chỉnh sửa hay xóa bất kỳ phần nội dung nào khác của các task không liên quan.
3. Chỉ trả về kết quả là toàn bộ nội dung văn bản sau khi đã chỉnh sửa xong. Không có lời chào, lời giải thích hay bất kỳ văn bản giao tiếp nào khác.
"""
...
def edit_raw_tasks_with_ai(current_json_str, user_request, provider, api_key):
    ...
    and returns the updated list of raw tasks as a JSON string.
    """
    base_dir = os.getcwd()
